...
const Tabs: React.FC<TabProps> = ({
    ...
    return (
        <div className={`flex flex-col w-full ${isDarkMode ? 'dark bg-[#0e0f11]' : ''} ${className}`}>
            <div className={`w-full relative flex items-start justify-center ${isDarkMode ? 'bg-transparent' : ''}`}>
                <div className="relative w-full">
                    {/* Hover Highlight */}
                    <div
                        className="absolute transition-all duration-300 ease-out rounded-[6px]"
                        style={{
                            ...hoverStyle,
                    ...
                    <div className="relative flex space-x-[6px] items-center">
                        {tabs.map((tab, idx) => (
                            <div
                                ...
                                style={{ padding: tabPadding, color: idx === activeIndex ? activeColor : inactiveColor, fontSize: tabFontSize }}
            ...
            <div className={`mt-4 ${contentClassName}`}>
                {React.Children.map(children, (child, idx) => (
                    <div key={idx} style={{ display: idx === activeIndex ? 'block' : 'none' }}>
                        {child}
                    </div>
                ))}
            </div>
...
const ScrollSection: React.FC<ScrollSectionProps> = ({ id, section, activeSection, setActiveSection, children }) => {
...
const CardContent2: React.FC<CardContent2Props> = ({ index, total, title, tabs }) => (
    <div className="space-y-8 flex flex-col w-full">
        <div className="mt-4 w-full flex flex-col sm:items-start items-center">
            <p className="text-gray-400 tracking-widest">PHASE {index} / {total}</p>
            <h1 className="text-3xl sm:text-4xl font-serif mt-4">{title}</h1>
        </div>

        <div className="w-full">
            <Tabs tabs={tabs.map(step => step.title || "")} contentClassName="p-4 border rounded-lg">
                {tabs.map((step, idx) => (
                    <div key={idx}>{step.description}</div>
                ))}
            </Tabs>
        </div>
    </div>
...
const ScrollableSection: React.FC<ScrollableSectionProps> = ({ sectionData }) => {
...
const MobileSection: React.FC<MobileSectionProps> = ({ sectionData }) => (
    <div className="w-full h-fit block">
        {sectionData?.map((section, idx) => (
            <div key={idx} className="mb-12 w-full px-4">
...
export default function Example5() {
    ...
    return (
        <main className="container mx-auto flex min-h-screen flex-col items-center justify-between">
            {isDesktop ? (
                <ScrollableSection sectionData={frameWork} />
            ) : (
                <MobileSection sectionData={frameWork} />
            )}
        </main>
    );
}
...
//     console.log("key", key)
//     useEffect(() => {
//         if (inView) setActiveSection(key)
//     }, [inView])
//
//     return (
//         <div
